...
def scrap(channel, post):
    ...
    video = soup.find(
        "video", attrs={"class": "tgme_widget_message_video js-message_video"}
    )
...
def run(tchannel, start_post, img_folder, tweet_text, tweets_per_h):
    ...
    while True:
        ...
        if content.endswith(".jpg"):
            ...
            nomedia_cont = 0
            time.sleep(
                twposter.tweets_hour(tweets_per_h)
            )  # Example: 4 tweets/h = 900 s = 15 min
            ...
            if nomedia_cont < verify_number:
                ...
                time.sleep(
                    twposter.tweets_hour(tweets_per_h) / 3
                )  # Waiting to check for a new telegram post
